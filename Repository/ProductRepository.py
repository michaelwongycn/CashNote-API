...
class ProductRepository:
    ...
    def GetProductAndNonActiveById(product_id):
        connection = Utility.get_connection()
        cursor = connection.cursor()

        query = "SELECT * FROM product WHERE product_id = ? AND (product_status = 'A' OR product_status = 'N')"
        params = [product_id]
        cursor.execute(query, params)
    ...
    def NonActiveProduct(product_id):
        connection = Utility.get_connection()
        cursor = connection.cursor()

        query = "UPDATE product SET product_status = 'N' WHERE product_id = ? AND product_status = 'A'"
        params = [product_id]
        cursor.execute(query, params)

        query = "UPDATE product_detail SET product_detail_status = 'N' WHERE product_id = ? AND product_detail_status = 'A'"
        params = [product_id]
        cursor.execute(query, params)
